...
qry = db.execute('SELECT * FROM hnarep')
data = qry.fetchall()
for row in data:
    r_set.append(row)
...
def add_to_database():
    ...
    db.execute('INSERT INTO hnarep (id, first_name, last_name, email, car_make, car_model, car_year, car_price) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', values)
    db.commit()
    refresh_table()
...
def delete_from_database():
    ...
    if "," in id_to_delete:
        ...
        db.execute('DELETE FROM hnarep WHERE id > ? AND id < ?', (int(ids[0])-1, int(ids[1])+1))
    else:0
        db.execute('DELETE FROM hnarep WHERE id = ?', (id_to_delete,))
    db.commit()
    refresh_table()
